...
import vsu.cs.solitaire.cards.Card;
import vsu.cs.solitaire.cards.Deck;
...
public class SolitaireBoard extends JPanel implements MouseListener, MouseMotionListener {
	...
	private void drawBackground(Graphics g) {
		...
		int xN = rect.x + (rect.width - metrics.stringWidth("Новая")) / 2;
		int yN = rect.y + rect.height / 2 - metrics.getHeight() + metrics.getAscent();
		g.drawString("Новая", xN, yN);

		int xG = rect.x + (rect.width - metrics.stringWidth("Игра")) / 2;
		int yG = rect.y + (rect.height / 2) + metrics.getAscent();
		g.drawString("Игра", xG, yG);
	}
	...
	private void checkWin() {
		...
		if (won) {
			setEnabled(false);
			String[] options = new String[]{"Играть снова!", "Выход"};
			int r = JOptionPane.showOptionDialog(this, "Поздравляю, вы выиграли! Желаете начать заново?",
					"Поздравления", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null,
					options, options[0]);
